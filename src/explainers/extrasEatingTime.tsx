...
const EATING_TIME_MULTIPLIER = 1;
function getAverageEatingTime(base: number, n: number) {
	// base - base eating time
	// n - max number of extra orders
	let averageEatingTime =
		(n
			? (1 - Math.exp((-n / 5) * EATING_TIME_MULTIPLIER * base)) *
			  (base +
					(5 / n -
						(EATING_TIME_MULTIPLIER * base) /
							(Math.exp((n * EATING_TIME_MULTIPLIER * base) / 5) - 1)))
			: 0) +
		Math.exp((-n / 5) * EATING_TIME_MULTIPLIER * base) *
...
export default ExtrasEatingTime;
