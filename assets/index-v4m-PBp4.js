...
`,Ye=Aa.split(`\r
`).map(e=>{var t,o;const[a,s,l,n]=e.split(","),r=Number(a),i=((t=l==null?void 0:l.split(":"))==null?void 0:t.map(c=>Number(c)))??[],u=((o=n==null?void 0:n.split(":"))==null?void 0:o.map(c=>Number(c)))??[];return{ID:r,Name:s,RequiredProcesses:i,IngredientProviders:u}});console.log({Unlocks:Ye});var Na=0;function g(e,a,s,l,n,r){var i,u,t={};for(u in a)u=="ref"?i=a[u]:t[u]=a[u];var o={type:e,props:t,key:s,ref:i,__k:null,__:null,__b:0,__e:null,__d:void 0,__c:null,constructor:void 0,__v:--Na,__i:-1,__u:0,__source:n,__self:r};if(typeof e=="function"&&(i=e.defaultProps))for(u in i)t[u]===void 0&&(t[u]=i[u]);return d.vnode&&d.vnode(o),o}const Ea=[!0,!1],Oa=[!0,!1];function Da(e){return[5,8,10,12,13,15,16,18,19,20,21,22,23,24,25][e-1]+1}const Ia=["Breakfast",,,"Breakfast Eggs",,"Exclusive","Breakfast Extras",,,"Breakfast Beans",,,"Pumpkin Seed"],za=({seed:e,day:a=1,blueprintCount:s,upgradeChance:l,ownedAppliances:n,cardsByDay:r})=>{console.log({ownedAppliances:n});const i=new xa(e,l);for(let p=0;p<=a;p++)if(r[p]){const f=Ye.filter(h=>h.Name===r[p])[0];f&&i.addCard(f)}const u=Object.entries(n).filter(p=>p[1]).map(p=>p[0]).map(p=>E.filter(f=>f.Name===p)[0]);for(const p of u)p&&i.OwnedAppliances.push(p);console.log(i.OwnedAppliances);let t=[];for(const p of Oa)for(const f of Ea)if(t.push({spawnInside:p,playerInside:p?void 0:f,blueprintCount:s}),p)break;const o=[];let c=[[],[],[],[]];for(let p=0;p<t.length;p++)for(let f=0;f<t.length;f++){o.push([t[p],t[f]]);const h=[t[p].spawnInside,t[p].playerInside,t[f].spawnInside,t[f].playerInside];for(let v=0;v<h.length;v++)c[v].length&&c[v].at(-1).val===h[v]?c[v].at(-1).run+=1:c[v].push({val:h[v],run:1})}let F=[];const m=s,_=4,T=Da(a);console.log({day:a,CAP:T});for(let p=0;p<T;p++){const f=[];for(const h of p?o:o.map(v=>[v[0]])){const v=[...h];p&&v.splice(-1,1,{...v.at(-1),blueprintCount:m+_}),f.push(i.getAppliances(v,a).map(C=>C.Name))}if(F.push(f),p)for(let h=0;h<o.length;h++)o[h].push(o[h][o[h].length-1])}const S=(p,f)=>{switch(p){case 0:return f?"Spawn Inside":"Spawn Outside";case 1:return f===void 0?"":f?"Stand Inside at End of Day":"Everyone Outside at End of Day";case 2:return f?"Spawn Inside for Rerolls":"Spawn Outside for Rerolls";case 3:return f===void 0?"":f?"Someone Inside when Rerolling":"Everyone Outside when Rerolling";default:throw new Error("Too Many Headers")}};return g("table",{children:[g("thead",{children:c.map((p,f)=>g("tr",{children:[!f&&g("th",{colSpan:1,rowSpan:4,children:"Reroll #"}),p.map(h=>h.val!==void 0&&g("th",{colSpan:h.run,rowSpan:S(f,h.val).indexOf("Spawn Inside")>-1?2:1,scope:"colgroup",children:S(f,h.val)}))]}))}),F.map((p,f)=>g("tr",{children:[g("th",{children:f||"Spawn"}),f?p.map(h=>g("td",{colspan:1,children:h.map((v,C)=>g("span",{class:C<m?"":"ghost",children:v+(C<h.length-1?", ":"")}))})):g("td",{style:"text-align:center;",colSpan:16,children:p[0].join(", ")})]}))]})},Ze=["Blueprint Cabinet","Plates","Research Desk"],oe={};for(const e of Ze)oe[e]=!1;oe["Blueprint Cabinet"]=!0;const Ha="t4tmhm8r",Ua=()=>{const[e,a]=Z(Ha),[s,l]=Z(1),[n,r]=Z(oe);let i=[];switch(e){case"az95tz5z":i=["Stir Fry","Broccoli","Steak Stir Fry","Mashed Potato","Mushroom Stir Fry","Affordable","Ice Cream","Herd Mentality","Roast Potato","Health and Safety","Victorian Standards","Instant Service","All You Can Eat","Bamboo","Chips"];break;case"t4tmhm8r":i=Ia;break;default:i=[];break}const u={seed:e,day:s,upgradeChance:e.indexOf("az")>-1?.25:0,blueprintCount:e.indexOf("az")>-1?7:5,ownedAppliances:n,cardsByDay:i},t=F=>{l(Number(F.target.value))},o=F=>{a(F.target.value)},c=F=>{const m=F.target;r(_=>({..._,[m.id]:m.checked}))};return g(x,{children:[g("h3",{children:["Seed: ",g("input",{value:e,onChange:o})]}),g("label",{children:"Shop after completing day "}),g("input",{style:"border:none;width:2.5em",type:"number",value:s,onChange:t}),g("div",{children:Ze.map(F=>g("div",{style:"margin:5	px",children:[g("label",{for:F,children:["Own ",F,"?"]}),g("input",{type:"checkbox",checked:n[F],id:F,onChange:c})]}))}),g(za,{...u})]})};ra(g(Ua,{}),document.getElementById("app"));
