var J=Object.defineProperty;var Q=(u,c,f)=>c in u?J(u,c,{enumerable:!0,configurable:!0,writable:!0,value:f}):u[c]=f;var p=(u,c,f)=>(Q(u,typeof c!="symbol"?c+"":c,f),f);(function(){"use strict";var u=(e=>(e[e.Generic=0]="Generic",e[e.Dish=1]="Dish",e[e.PrimaryTheme=3]="PrimaryTheme",e[e.SecondaryTheme=4]="SecondaryTheme",e[e.Special=6]="Special",e))(u||{}),c=(e=>(e[e.Base=0]="Base",e[e.Main=1]="Main",e[e.Extra=2]="Extra",e[e.Side=3]="Side",e[e.Starter=4]="Starter",e[e.Dessert=5]="Dessert",e[e.Null=6]="Null",e))(c||{}),f=`373996608,Ice Cream,1,,,,-1533430406,False,False,5\r
...
-373772933,Simple Cloth Table,5,,,,,False,False,6`;const C=f.split(`\r
`).filter(e=>e).map(e=>{var v,w;const[a,s,l,F,n,i,t,o,d,h]=e.split(","),T=Number(a),m=Number(l),S=F!=null&&F.length?F.split(":").map(k=>Number(k)):[],r=n.length?n.split(":").map(k=>Number(k)):[],g=i!=null&&i.length?(v=i==null?void 0:i.split(":"))==null?void 0:v.map(k=>Number(k)):[],b=t!=null&&t.length?(w=t==null?void 0:t.split(":"))==null?void 0:w.map(k=>Number(k)):[],Y=o==="True",_=d==="True",$=Number(h);return{ID:T,Name:s,UnlockGroup:m,Requires:S,BlockedBy:r,RequiredProcesses:g,IngredientProviders:b,isMain:Y,isStarterOrSide:_,DishType:$}});function B(e,a=!0){const s=a&&e.some(l=>l.Name==="Community");return C.filter(l=>!(e.some(F=>F.ID===l.ID)||l.BlockedBy.some(F=>e.some(n=>n.ID===F))||l.Requires.some(F=>!e.some(n=>n.ID===F))||l.isStarterOrSide&&!e.some(F=>F.isMain)||!s&&l.DishType===c.Base)).sort((l,F)=>l.ID-F.ID)}u.Special,c.Null,u.Special,c.Null,u.Special,c.Null,C.filter(e=>e.Name==="Community")[0],u.Special,c.Null,u.Special,c.Null,["Breakfast","Burgers","Hot Dogs","Fish","Pies","Salad","Stir Fry","Pizza","Turkey"].map(e=>C.filter(a=>a.Name===e)[0]);class y{constructor(a,s){p(this,"fixedSeed");p(this,"instance");p(this,"random");typeof a=="string"&&(a=G(a)),this.fixedSeed=a,this.instance=s,this.random=new I(a+Math.imul(124192293,s))}useSubcontext(a){return new y(this.fixedSeed,124192293*a+this.instance)}}class I{constructor(a){p(this,"seed");const s=t=>Math.imul(1812433253,t)+1>>0,l=a>>0,F=s(l),n=s(F),i=s(n);this.seed=[l,F,n,i]}get value(){let a=this.seed.shift(),s=this.seed[2];return a^=a<<11,a^=a>>>8,s^=s>>>19,s=(s^a)>>0,this.seed.push(s),s}range(a,s){return(this.value>>>0)%(s-a)+a}get valueFloat(){return(this.value<<9>>>0)/4294967295}select(a){return a[this.range(0,a.length)]}}function G(e){const s=new TextEncoder().encode(e);let l=5381,F=s.length;for(;F>0;){let n=--F;l=(l<<5)+l+s[n]}return l>>0}class x{constructor(){p(this,"encourageGroups",[u.Special])}GetOptions(a,s,l){const F=l.random;let n=B(a),i=B(a,!1).length+1;for(console.log({autumn:n.length,non:i-1}),a.some(r=>r.Name==="Turbo")?s!==5&&(i+=n.length+1):a.some(r=>r.Name==="Community")?s===5&&(i+=n.length+2):console.error("This Setting is not implemented yet"),console.log({pad:i});i--;)F.value;let t=E(n,F);console.log(t.map(r=>r.Name).join(", "));const o=F.valueFloat<(a.some(r=>r.Name==="Turbo")?.1:.5),d=!a.some(r=>r.Name==="Community")||s===5;o&&(console.log("Prefer Priority"),t.sort((r,g)=>{let b=0;return(d&&r.Requires.length||this.encourageGroups.includes(r.UnlockGroup))&&b--,(d&&g.Requires.length||this.encourageGroups.includes(g.UnlockGroup))&&b++,b}));const h=s===5?u.PrimaryTheme:u.Dish,T=s===5?u.PrimaryTheme:a.some(r=>r.Name==="Community")?u.Dish:u.Generic;let m=null,S=null;for(const r of t)if(!(r.UnlockGroup!==h&&r.UnlockGroup!==T)&&(m===null||m.UnlockGroup!==h&&r.UnlockGroup===h?m=r:(S===null||S.UnlockGroup!==T&&r.UnlockGroup===T)&&(S=r),m&&m.UnlockGroup===h&&S&&S.UnlockGroup===T))break;return[m,S]}}function E(e,a){return e.map(l=>[l,a.valueFloat]).sort((l,F)=>l[1]-F[1]).map(l=>l[0])}class A{constructor(a){p(this,"cards");p(this,"unlockPack");p(this,"seed");this.cards=[],this.unlockPack=new x,this.seed=a}addCard(a){a!==void 0&&this.cards.push(a)}getUnlockOptions(a){const l=H(848292,a,this.seed).useSubcontext(1);return this.unlockPack.GetOptions(this.cards,a,l)}}function H(e,a,s){return new y(s,e*1231231+a)}class O{constructor(a){p(this,"seed");p(this,"mapSize");this.seed=a,this.mapSize=this.getMapSize()}getMapSize(){const s=new y(this.seed,5078598).useSubcontext(0).random.range(0,4);let l;switch(s){case 0:l=2;break;case 1:l=1;break;case 2:l=3;break;case 3:l=4;break;default:throw new Error}return l}}var L=`505496455,False,True,Heated Mixer,False,False,,2087693779:-523839730:1972879238,64,0,0,False\r
...
-1995745069,False,False,SyncModsViewHolder,False,False,,,0,0,0,False`;const q=["ID","IsPurchasable","IsPurchasableAsUpgrade","Name","SellOnlyAsDuplicate","SellOnlyAsUnique","RequiresForShop","RequiresProcessForShop","ShoppingTags","ThemeRequired","ShopRequirementFilter","StapleWhenMissing"],U=["number","boolean","boolean","string","boolean","boolean","number[]","number[]","number","number","number","boolean"];L.split(`\r
`).map(e=>{const a=e.split(",");let s={};for(let l=0;l<a.length;l++){let F;switch(U[l]){case"number":F=Number(a[l]);break;case"boolean":F=a[l]==="True";break;case"number[]":a[l]===""?F=[]:F=a[l].split(":").map(n=>Number(n));break;default:F=a[l]}s[q[l]]=F}return s}).filter(e=>e.IsPurchasable||e.IsPurchasableAsUpgrade).length;var z=self;let M=0;const D=60;let P=!1;z.onmessage=function(e){console.log("Worker: Message received from main script"),e.data.type==="start"?(P=!0,M=0,W(e.data.data),setTimeout(()=>P=!1,1e3*60*D)):(console.log("breaking search"),P=!1)};async function W({goalCards:e,goalAppliances:a=[],mapSizes:s,maxSeeds:l=10,partial:F=!0}){let n=0;const i=new MessageChannel;let t;i.port2.onmessage=m=>{t()};let o="az2mpjp3";const d=e[0].cards;let h=[-1/0,[]],T=l;for(;P&&T;){n++;const m=new Promise(g=>{t=g});for(i.port1.postMessage(null),await m,o="az";o.length<8;)o+=R[Math.floor(Math.random()*R.length)];const S=new O(o).mapSize;if(!s.includes(S))continue;const r=K(o,d,e,a,F);if(r[0]>=h[0]){if(!Number.isFinite(r[0]))continue;h=r,T--;const g=JSON.stringify({seed:o,metric:[r[0],r[1]]});console.log(g),V({seed:o,cards:r[1],blueprints:[]})}}console.log(`${M} distinct seeds checked in ${D} minutes over ${n} tries.`)}function V(e){postMessage({type:"result",data:e})}C.filter(e=>e.Name==="Ice Cream")[0];function K(e,a,s,l,F=!1){const n=new A(e);for(const o of a)n.addCard(o);let i=0,t=1;for(;t<15;t++){const o=n.getUnlockOptions(t),d=N(o[0],s[t]),h=N(o[1],s[t]);let T=-1;if(d>-1&&h>-1)d<h?T=0:h<d?T=1:T=Math.floor(Math.random()*2);else if(d===-1&&h===-1){if(!F||t<2)return console.log(o.map(m=>m.Name).join(", ")+` on day ${t} both suck.`),[-1/0,[]];T=Math.floor(Math.random()*2),i++}else d>=0?T=0:T=1;if(!o[T]){debugger;throw console.log({options:o}),new Error}n.addCard(o[T])}return[t-i,n.cards.map(o=>o.Name)]}const R="abcdefghijklmnopqrstuvwxyz123456789";function j(e,a){for(let s=0;s<e.length;s++)if(e[s].ID===a.ID)return s;return-1}function N(e,a){return a.include?j(a.cards,e):a.cards.some(s=>s.ID===e.ID)?-1:0}})();
